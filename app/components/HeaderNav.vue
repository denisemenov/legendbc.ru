      ...
      <div class="flex flex-row items-center justify-between">
        <div class="flex flex-col items-center lg:items-start">
          ...
          <div class="mt-4 lg:mt-2">
            ...
            <NuxtLinkLocale to="tel:+79166244545" class="flex justify-center items-center hover:opacity-80 transition">
              <NuxtImg src="/images/phone.svg" alt="Телефон" class="w-4 h-4 block" />
...
class="lg:hidden top-10 right-4 border-2 border-white p-2.5 z-[999] transition-colors"
          :class="{ 'bg-gold-500': isMenuOpen }" @click="toggleMenu">
...
class="w-full h-0.5 bg-white transition-transform duration-300"
            ...
            <span class="w-full h-0.5 bg-white transition-opacity" :class="{ 'opacity-0': isMenuOpen }" />
            <span
class="w-full h-0.5 bg-white transition-transform duration-300"
              :class="{ '-rotate-45 -translate-y-[7px]': isMenuOpen }" />
          </div>
